...
/**
 * This class manages all available Thumbnailers. Its purpose is to delegate a File to the
 * appropriate Thumbnailer in order to get a Thumbnail of it. This is done in a fall-through manner:
 * If several Thumbnailer can handle a specific filetype, all are tried until a Thumbnail could be
 * created.
 * <p/>
 * Fill this class with available Thumbnailers via the registerThumbnailer()-Method. Then call
 * generateThumbnail().
 * @author Benjamin
 */
@Slf4j
...
*/
    ...
    /**
     * Generate a Thumbnail. The output file name is generated using a hashing scheme. It is
     * garantueed that an existing Thumbnail is not overwritten by this.
     * @param input Input file that should be processed.
     * @return Name of Thumbnail-File generated.
     * @throws IOException
     * @throws ThumbnailerException
     */
    public File createThumbnail(File input) throws IOException, ThumbnailerException {
     ...
     * Thumbnailers may make a difference: First added Thumbnailers are tried first, if one fails,
     * the next (that claims to be able to treat such a document) is tried. (Thumbnailers that claim
     * to treat all MIME Types are tried last, though.)
     ...
     * <p/>
     * This functions should be called before termination of the program, and Thumbnails can't be
     ...
     * returns an image.
     * <p/>
     * MIME-Detection narrows the selection of Thumbnailers to try: <li>First all Thumbnailers that
     * declare to accept such a MIME Type are used <li>Then all Thumbnailers that declare to accept
    ...
    public String[] getAcceptedMIMETypes() {
        if (thumbnailers.containsKey(ALL_MIME_WILDCARD)) {
            ...
            return thumbnailers.keySet().toArray(new String[thumbnailers.size()]);
        }
    }
