 ...
 * <p/>
 * Contract: <li>It is possible to put several identical key-value pairs (i.e. where key and value
 * is equal) <li>entrySet is not supported. Instead, it can be iterated over all entries.
 * @param <K> Key
 * @param <V> Value
 */
public class ChainedHashMap<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>> {
    ...
    /**
     * Iterate over all elements in the table. Note that this currently copies them into a
     * collection, so concurrent modification will not be taken into account (there will be no
